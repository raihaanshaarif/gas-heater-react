...
suburbs.forEach(suburb => {
  ...
  // Skip armadale as it's already been updated
  if (suburb === 'armadale') {
    console.log(`✓ Skipped armadale (already updated)`);
    return;
  }
  ...
  // Pattern: closing </div> from PromoLayout, then the old Why Choose Us section, then opening <div> for next section
  const oldPattern = /        <\/div>\n                {\/\* Service Area Details \*\/}\n                <div className="mt-\[40px\]">\n                  <h3 className="ttitle text-\[24px\] md:text-\[32px\] mb-\[24px\]">Why Choose Us for [^<]+ Gas Heating<\/h3>\n                  <div className="grid md:grid-cols-2 gap-\[20px\] text-\[14px\]">[\s\S]*?<\/div>\n                <\/div>\n        <div className="section__inner bg-no-repeat bg-center lazyload"/;
